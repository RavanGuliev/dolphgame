...
<template>
  ...
  <div class="sticky top-0 z-40 transition-shadow duration-300 ease-out" :class="isScrolled ? 'shadow-[0_8px_24px_-12px_rgba(0,0,0,0.25)]' : 'shadow-none'">
    ...
    <header class="bg-white/95 dark:bg-ink-900/95 backdrop-blur border-b border-ink-200 dark:border-ink-800" data-dolph-nav>
      <div class="max-w-7xl mx-auto px-4 md:px-6 py-3 md:py-4 flex items-center gap-2 md:gap-8">
        <!-- Mobile burger -->
            ...
            <button v-for="n in notifications" :key="n.id" @click="readNotification(n.id, n.type)" type="button" class="w-full text-left group rounded-xl bg-ink-50 dark:bg-ink-900 border border-ink-200 dark:border-ink-800 p-3.5 hover:border-brand-500/50 transition">
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="mt-1 text-[11px] text-ink-400 flex items-center gap-1"><i class="fal fa-clock"></i> {{ formatDateTime(n.created_at) }}</div>
                </div>
              </div>
            </button>
